...
def main():
    ...
    parser.add_argument('--expfile', default='exp_configs/sol_zombie_horde.yaml', help='Experiment config to run.')
    parser.add_argument('--exp_name', default='exp', help='Experiment name.')
    parser.add_argument('--wandb_proj', default='test', help='Wandb project to save results in.')
    parser.add_argument('--debug', action='store_true', help='Use single process and minimum workers to enable pdb.')
    parser.add_argument('--mode', default='local', choices=['local', 'slurm'], help='Run locally or submit Slurm jobs.')
    parser.add_argument('--local_results_dir', default='./results', help='Where to save experiment results when running locally.')
    # these are used if submitting jobs on Slurm
    parser.add_argument('--script', default='single_node_template.sbatch', help='Sbatch script template.')
    parser.add_argument('--partition', default='', help='Slurm partition.')
    parser.add_argument('--num_gpus', default=1, help='Stays at 1 in all our experiments.')
    parser.add_argument('--num_cpus', default=50, help='Set to around num_workers.')
    parser.add_argument('--days', default=3, help='Time limit for job.')
    parser.add_argument('--seeds', default=5, help='Number of seeds (each job will be an array equal to number of seeds.')
    ...
    if 'nethack' in env_name or 'minihack' in env_name:
        BASE_CMD = 'python -m sf_examples.nethack.train_nethack'
    elif 'mujoco' in env_name:
        BASE_CMD = 'python -m sf_examples.mujoco.train_mujoco'
    else:
    ...
    if args.mode == 'local':
        ...
        cmd += f' --train_dir {args.local_results_dir} --with_wandb False'
        if args.debug:
