...
def _ttyrec_generator(
    batch_size, seq_length, rows, cols, load_fn, map_fn, ttyrec_version, game_data_map
):
    ...
    while np.any(
        ...
        assert key_vals[5][0] == "gameids"
        game_data = np.array([[game_data_map[gid] for gid in col] for col in key_vals[5][1]])

        yield dict(key_vals) | {"game_data": game_data}
...
class TtyrecDataset:
    ...
    def __init__(
        ...
        core_sql = """
            SELECT ttyrecs.gameid, ttyrecs.part, ttyrecs.path, games.role, games.race, games.gender, games.align   
            FROM ttyrecs
        ...
        with nld.db.connect(self.dbfilename) as conn:
            ...
            for row in c.execute(core_sql, sql_args):
                # The last 4 are the extra data
                self._games[row[0]].append(row[1:7])
    ...
    def get_paths(self, gameid):
        return [path for _, path, *_ in self._games[gameid]]
    ...
    def __iter__(self):
        ...
        game_data_map = {
            k: "-".join(game_data[0][2:]) for k, game_data in self._games.items()
        }
