...
class NLE(gym.Env):
    ...
    def __init__(
        ...
        if render_mode == "full":
            for key in ("chars", "colors", "tty_chars"):
                if key not in self._observation_keys:
                    self._observation_keys.append(key)
    ...
    def render(self):
        ...
        if mode == "full":
            ...
            message = bytes(self.last_observation[message_index]).decode('latin-1')
            print(message)
            print()
            ...
            try:
                ...
                for letter, line in zip(inv_letters, inv_strs):
                    if np.all(line[:-1] == 0):
                        break
                    line_str = line.tobytes().decode("utf-8")
                    end = "" if (line_str and line_str[-1]=="\n") else "\n"
                    print(
                        letter.tobytes().decode("utf-8"), "-", line_str, end=end
                    )
            ...
            blstats = self.last_observation[self._observation_keys.index("tty_chars")][-2:]
            for row in blstats:
