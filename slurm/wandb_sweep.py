...
def main():
    ...
    parser.add_argument('--wandb_agent_cmd')
    parser.add_argument('--script', default='single_node_template_wandb.sbatch')
    ...
    BASE_CMD = args.wandb_agent_cmd + " --count 1"
    ...
    if not num_jobs_running() == MAX_JOBS:            
        subprocess.run(["sbatch"], input=script_template.encode())
