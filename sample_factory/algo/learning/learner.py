...
from sf_examples.nethack.utils.nle_tokenizer.tokenizer import DND_DETOKENIZER, DND_TOKENIZER

from sample_factory.algo.utils.cython.controller_reward_computation import compute_controller_reward_sums_cython
...
class Learner(Configurable):
    def __init__(
        ...
        self.goal_completions = defaultdict(float)
    ...
    def init(self) -> InitModelData:
        if self.cfg.exploration_loss_coeff == 0.0:
            ...
            if self.cfg.with_sol:
                ...
                if self.cfg.sol_option_exploration_scales:
                    self._sol_option_exploration_scales = torch.tensor(
                        [float(x) for x in self.cfg.sol_option_exploration_scales.split(",")],
                        dtype=torch.float32,
                    )
                else:
                    self._sol_option_exploration_scales = None
            else:
        ...
        if self.cfg.use_ddp:
            # DDP forgets actor_critic's methods, so wrap each module separately
            # this also inserts a '.module' in weight named which we will have to remove
            # when copying them to policy worker's actor_critic.
            self.actor_critic.encoder = DDP(self.actor_critic.encoder, device_ids=[int(rank)])
            self.actor_critic.core = DDP(self.actor_critic.core, device_ids=[int(rank)])
            self.actor_critic.critic_linear = DDP(self.actor_critic.critic_linear, device_ids=[int(rank)])
            self.actor_critic.action_parameterization = DDP(self.actor_critic.action_parameterization, device_ids=[int(rank)])
    ...
    def _save_impl(self, name_prefix, name_suffix, keep_checkpoints, verbose=True) -> bool:
        if self.cfg.use_ddp:
            # if using DDP, only save for rank 0
            rank = int(os.environ.get("RANK"))
            if rank == 0:
                log.debug(f"[DDP] Proceeding with checkpoint saving for rank {rank} process")
            else:
                log.debug(f"[DDP] Skipping checkpoint saving for rank {rank} process")
                return True
    ...
    def _entropy_exploration_loss_sol(
    ...
    def _compute_vtrace_sol(
        ...
        valids = torch.ones((num_trajectories * recurrence,), dtype=torch.bool)
        ...
        seen_first_controller = torch.zeros(num_trajectories, dtype=torch.bool)
        ...
        for i in reversed(range(self.cfg.recurrence)):
            ...
            if self.cfg.sol_bootstrap_on_option_switch:
                if i < self.cfg.recurrence - 3:
                    ...
                    trajs_with_changed_options = (
                            ...
                            (policy_indicators[i::recurrence] != policy_indicators[(i + 2)::recurrence]).any(dim=-1)
                    )
                    # for any trajectories where the option switched, reset the base case so that bootstrapped returns are applies
                    is_base_case_handled[current_policies_one_hot] = is_base_case_handled[
                                                                         current_policies_one_hot] & ~trajs_with_changed_options
            ...
            next_values.view(-1)[base_case_indices.view(-1)] = (
                                                                       ...
                                                                       - rewards_cpu[i:: recurrence][
                                                                           base_case_indices_any]
                                                               ) / gamma
            ...
            next_vs.view(-1)[current_policies_one_hot.view(-1)] = (
                    ...
                    * curr_vtrace_c
                    * (next_vs.view(-1)[current_policies_one_hot.view(-1)] - next_values.view(-1)[
                current_policies_one_hot.view(-1)])
            )
        ...
        with torch.no_grad(), self.timing.add_time("advantages_returns"):
            if self.cfg.with_vtrace:
                ...
                if self.cfg.with_sol:
                    ...
                    policy_indicators = mb.normalized_obs['current_policy_vec'].long().cpu()
                    ...
                    controller_indx = num_policies - 1
                    policy_indicators = policy_indicators.view(num_trajectories, recurrence, num_policies)
                    ...
                    # this is hacky and will break if we use reward clipping, so can remove if needed.  
            ...
            if self.cfg.with_sol:
                for p in torch.unique(policy_indicators, dim=0):
                    policy_mask = (policy_indicators == p).all(dim=-1).to(self.device)
                    adv_std, adv_mean = torch.std_mean(torch.masked_select(adv, valids & policy_mask), correction=0)
        ...
        with self.timing.add_time("losses"):
            ...
            if self.cfg.with_sol:
                ...
                exploration_loss = self.exploration_loss_func(
                    ...
                    valids,
                    num_invalids,
                    controller_action_dim=controller_action_dim,
                    controller_indices=policy_indicators[:, -1].to(self.device),
                    policy_indicators=policy_indicators.to(self.device),
                )
            else:
    ...
    def train(self, batch: TensorDict) -> Optional[Dict]:
        ...
        if num_invalids >= experience_size:
            ...
            if self.cfg.use_ddp:
                experience_size = experience_size * self.world_size
