...
class Learner(Configurable):
    ...
    def init(self) -> InitModelData:
        if self.cfg.exploration_loss_coeff == 0.0:
            ...
            if self.cfg.with_sol:
                self.exploration_loss_func = self._entropy_exploration_loss_sol
            else:
        ...
        # only legacy Motif reward supported for now. Should add others in a cleaner manner than was
        # done previously, using the same encoder type as the actor. 
        if self.cfg.llm_reward_type in ["motif_legacy"]:
            self._load_motif_reward_model()

        return model_initialization_data(self.cfg, self.policy_id, self.actor_critic, self.train_step, self.device)

    # load a pretrained Motif reward model
    def _load_motif_reward_model(self):
        # store everything together in a dict
        self.motif = {}
        checkpoints = self.get_checkpoints(join(self.cfg.motif_reward_dir, "checkpoint_p0"))
        assert len(checkpoints) > 0
        reward_model_ckpt = self.load_checkpoint(
            checkpoints, self.device
        )

        if self.cfg.llm_reward_type == "motif_legacy":
            from sf_examples.nethack.models.reward_model import LegacyMotifRewardModel
            reward_model = LegacyMotifRewardModel(self.cfg)
        else:
            raise NotImplementedError("Only legacy NLE Motif model implemented for now")
        
        reward_model.load_state_dict(reward_model_ckpt["model"], strict=False)
        reward_model.add_mean_var(
            reward_model_ckpt["reward_mean"].item(),
            reward_model_ckpt["reward_var"].item(),
        )
        self.motif['reward_model'] = reward_model.to(self.device)

        # load threshold quantiles
        reward_csv_file = (
            f"{self.cfg.motif_reward_dir}/reward_metrics/train_norm_quantiles.csv"
        )
        ...
        quantile_index = np.where(
            ...
            == self.cfg.motif_eps_threshold_quantile
        )[0][0]
        self.motif['rew_eps_threshold'] = float(reward_quantile_info[-1][quantile_index])
    ...
    def _entropy_exploration_loss_sol(self, action_distribution, valids, num_invalids: int, controller_action_dim=None) -> Tensor:
        ...
        #entropies[controller_action_dim] *= self.cfg.sol_controller_exploration_scale
    ...
    def _symmetric_kl_exploration_loss(self, action_distribution, valids, num_invalids: int) -> Tensor:
        assert not self.cfg.with_sol, "Symmetric KL exploration loss not implemented with SOL"
    ...
    def _compute_vtrace_sol(
        ...
        for i in reversed(range(self.cfg.recurrence)):
            ...
            if self.cfg.sol_bootstrap_on_option_switch:
                if i < self.cfg.recurrence - 3:
    ...
    def _compute_motif_rewards(self, mb: AttrDict):
        
        # run observations through reward network
        llm_rewards_cpu = self.motif["reward_model"](mb).detach().cpu()

        # normalize by mean and variance precomputed offline
        llm_rewards_cpu = (
            llm_rewards_cpu - self.motif["reward_model"].mean
        ) / (self.motif["reward_model"].var) ** (1 / 2)

        # threshold based on quantiles precomputed offline
        llm_rewards_cpu = (
            llm_rewards_cpu > self.motif["rew_eps_threshold"]
        ) * llm_rewards_cpu
                        
        # normalize by inverse episodic message counts
        msg_count = mb['normalized_obs']['msg_count'].squeeze().cpu()
        msg_count_coeff = 1 / (
            msg_count ** (self.cfg.motif_beta_count_exponent)
        )
        return llm_rewards_cpu * msg_count_coeff
    ...
    def _calculate_losses(
        ...
        with torch.no_grad(), self.timing.add_time("advantages_returns"):
            if self.cfg.with_vtrace:
                ...
                # if we don't clone this, our assert fails on epoch > 1 because we fill values inplace.
                # the clone could be removed if we remove the assert but it's a good sanity check. 
                rewards_cpu = mb.rewards_cpu if self.cfg.num_epochs == 1 else mb.rewards_cpu.clone()
                dones_cpu = mb.dones_cpu

                if self.cfg.with_sol:
                    # fetch and reshape relevant tensors
                    policy_indx = mb.normalized_obs['current_policy'].long().cpu()
                    num_policies = mb.normalized_obs['rewards'].shape[1]

                    # if we're adding Motif rewards, we may add these to some options and not others
                    if self.cfg.llm_reward_type == "motif_legacy":
                        llm_rewards_cpu = self._compute_motif_rewards(mb)
                        for p in self.cfg.sol_options_to_add_llm_reward.split(','):
                            mask = policy_indx.squeeze() == int(p)
                            rewards_cpu[mask] += self.cfg.llm_reward_coeff * llm_rewards_cpu[mask]

                    controller_indx = num_policies - 1
                    policy_indx = policy_indx.view(num_trajectories, recurrence)
                    rewards_cpu = rewards_cpu.view(num_trajectories, recurrence)
                    obs_rewards = mb.normalized_obs['rewards'].view(num_trajectories, recurrence, num_policies)
                    obs_rewards = obs_rewards[:, 1:]
                    ...
                    # we couldn't do this earlier because it requires summing over future timesteps. 
                    indices = (policy_indx == controller_indx).nonzero()
                    policy_reward_sums = []
                    policy_reward_means = []
                    for b in range(num_trajectories):
                        row_indices = indices[indices[:, 0]==b, 1]
                        for i in range(len(row_indices) - 1):
                            reward_sums = torch.sum(obs_rewards[b][row_indices[i] + 1: row_indices[i+1]], dim = 0) 
                            reward_means = torch.mean(obs_rewards[b][row_indices[i] + 1: row_indices[i+1]], dim = 0) 
                            rewards_cpu[b][row_indices[i]] = reward_sums[controller_indx]
                            policy_reward_sums.append(reward_sums.cpu())
                            policy_reward_means.append(reward_means.cpu())
                                    

                        # for the last controller action, we can't compute the full reward because the trajectory is truncated,
                        # so we either take the average of whatever rewards are there (should be close to unbiased) or set to zero. 
                        # an alternative would be to handle the base case earlier in the V-trace function.
                        if row_indices[-1] < obs_rewards.shape[1] - 1:
                            reward_sums = torch.sum(obs_rewards[b][row_indices[-1] + 1:], dim = 0)
                            reward_means = torch.mean(obs_rewards[b][row_indices[-1] + 1:], dim = 0)
                        else:
                            reward_sums = torch.zeros(num_policies)
                            reward_means = torch.zeros(num_policies)
                        rewards_cpu[b][row_indices[-1]] = reward_sums[controller_indx]
                        policy_reward_means.append(reward_means.cpu())
                        policy_reward_sums.append(reward_sums.cpu())

                    policy_indx = policy_indx.view(-1)
                    rewards_cpu = rewards_cpu.view(-1)

                    # we save this to use with the ISO loss later
                    policy_reward_means = torch.stack(policy_reward_means)
                    policy_reward_sums = torch.stack(policy_reward_sums)
                    
                    # now compute advantages and value targets for all policies 
                    adv, vs = self._compute_vtrace_sol(
                        ratios_cpu,
                        values_cpu,
                        dones_cpu,
                        rewards_cpu,
                    ...
                    if self.cfg.llm_reward_type == "motif_legacy":
                        llm_rewards_cpu = self._compute_motif_rewards(mb)
                        rewards_cpu += self.cfg.llm_reward_coeff * llm_rewards_cpu 

                    adv, vs = self._compute_vtrace_flat(
                        ratios_cpu,
                        values_cpu,
                        dones_cpu,
                        rewards_cpu,
                ...
                # using regular GAE
                assert not self.cfg.with_sol, "SOL not implemented for GAE yet"
                adv = mb.advantages
                targets = mb.returns

            if self.cfg.with_sol:
                for p in range(num_policies):
                    policy_mask = (policy_indx == p).to(self.device)
                    adv_std, adv_mean = torch.std_mean(torch.masked_select(adv, valids & policy_mask))
                    adv[policy_mask] = (adv[policy_mask] - adv_mean) / torch.clamp_min(adv_std, 1e-7)
            else:
        ...
        with self.timing.add_time("losses"):
            ...
            if self.cfg.with_sol:
                controller_action_dim = len(action_distribution.distributions) - 1
                # TODO: do something cleaner
                if self.cfg.sol_num_option_steps == -1:
                    controller_action_dim -= 1
                exploration_loss = self.exploration_loss_func(action_distribution, valids, num_invalids, controller_action_dim=controller_action_dim)
            else:
        ...
        if self.cfg.with_sol and self.cfg.sol_iso_loss_coeff > 0:
            controller_log_probs = action_distribution.distributions[-1].log_p[policy_indx==controller_indx]
            controller_iso_loss = 0

            for p in range(num_policies - 1):
                k = int(policy_reward_means.shape[0] * self.cfg.sol_iso_quantile)
                top_values, top_indices = torch.topk(policy_reward_means[:, p], k)
                controller_action_targets = torch.full((len(top_indices),), p, dtype=torch.long, device=self.device)
                controller_iso_loss += F.nll_loss(controller_log_probs[top_indices], controller_action_targets)

            loss_summaries.update(
                dict(controller_iso_loss=controller_iso_loss)
            )
            # we might want to combine the two losses outside this function
            policy_loss += self.cfg.sol_iso_loss_coeff * controller_iso_loss
