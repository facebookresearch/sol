...
class NonBatchedMultiAgentWrapper(Wrapper):
    ...
    def step(self, action: ListActions) -> Tuple[ListObservations, Sequence, SeqBools, SeqBools, Sequence[Dict]]:
        ...
        if terminated or truncated:  # auto-resetting
            if 'rewards' in obs.keys():
                # for hierarchical methods, rewards for different options are
                # recorded in obs, so need to keep them for the last time step.
                # this is important for envs where there is a big reward when
                # episode ends. 
                obs_rewards = obs['rewards'].copy()
                obs, info["reset_info"] = self.env.reset()
                obs['rewards'] = obs_rewards
            else:
                obs, info["reset_info"] = self.env.reset()
